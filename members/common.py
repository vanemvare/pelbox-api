...
try:
    log.info(f"Connecting to {env.str('DB_NAME')} database from members.common module")
...
def get_pelbox_settings(member_id):
    ...
    try:
        cur = conn.cursor()
        query = """
           	SELECT rd.id, rd.security_key, rd.user_security_key, rd.host, rd.member_id, rd.connected, bl.locked, bl.dismantle
           	FROM rpi_devices rd
