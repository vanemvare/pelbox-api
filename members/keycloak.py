...
class Keycloak:
    def __init__(self, admin_client_secret, member_client_secret):
    ...
    def verify_admin_token(self):
        ...
        """
        url = "http://localhost:8080/auth/admin/realms/pelbox/users?username=test&enabled=true"
        headers = {
    ...
    def register(self, member):
        ...
        url = "http://localhost:8080/auth/admin/realms/pelbox/users"
        payload = {
            "email": member.email,
            "enabled": "true",
    ...
    def member_login(self, member):
        ...
        """
        url = "http://localhost:8080/auth/realms/pelbox/protocol/openid-connect/token"
    ...
    def is_member_logged(self, access_token):
        ...
        url = f"http://localhost:8080/auth/admin/realms/pelbox/users/{id}/sessions"
        headers = {
            "Authorization": f"Bearer {self.access_token}"
        }
        response = requests.get(url, headers=headers)
    ...
    def get_member_id(self, username):
        ...
        url = f"http://localhost:8080/auth/admin/realms/pelbox/users?username={username}"
        headers = {
            "Authorization": f"Bearer {self.access_token}"
        }
